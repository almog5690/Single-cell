...
overdispersion_analysis <- function(data.type, force.rerun = FALSE,attribute = "selection") {
  ...
  for(i in 1:length(samples$organs)){
    ...
    young.ind = c(SC@meta.data$age %in% groups$young_ages) # index for cells that came from young mouses
    old.ind = c(SC@meta.data$age %in% groups$old_ages_1) # index for cells that came from old mouses
    old_ages = groups$old_ages_1
    if(sum(old.ind) == 0){
      old.ind = c(SC@meta.data$age %in% groups$old_ages_2)
      old_ages = groups$old_ages_2
    }
...
#                                               paste0("beta",attribute,"log_fc",sep = "_") = beta_log_fc,"pval_log_fc" = p_val_fc))
  ...
  return(DF_OD_res)
} # End overdispersion_analysis function 
