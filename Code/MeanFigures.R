...
draw_mean_figures <- function(data.types, fig.num, feature.types = c("selection", "gene.len"), 
  ...
  if(fig.num %in% c(1, 11))  #### Figure 1 (all) or 11 (fold-change)
  {
    ...
    for(feature.type in feature.types) # Here plot each feature vs. mean expression, not just selection
    {
      ...
      ggsave(paste0(analysis.figures.dir, "Mean.Figure", fig.num, '.', feature.type, '.png'), height = 6,width = 9)  # Modify name to get figure  
    }    
  }  
...
#      gene_selection = gene_features[[1]]
      ...
      for(i in 1:n.datas) {   
        ...
        gene_selc = gene_features[[feature.type]][gene_name %in% (SC_gene_name)] # filtering the selection score to genes that are found in the current tissue
