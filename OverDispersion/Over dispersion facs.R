...
DF_cor_len_disp = c()
df_fc_reg_facs = c()
len_OD_reg_data_sm = c()
selc_OD_reg_data_sm = c()
df_selc_reg_facs = c()
df_len_reg_facs = c()
for(i in 1:length(organs)){
  ...
  old.ind = c(SC@meta.data$age %in% c("18m","21m","24m")) # index for cells that came from old mouses
  ...
  for(k in cells_ind){
    ...
    test_file = paste("/tmp/DVT/DVT",organs[i],cell_types_categories[k],"same-mean.RData")
    if(!file.exists(test_file)){ # Checking if the file exist
      load(test_file)
    } else {
      # Old Markov chain file name
      old_file = paste0("/tmp/chains","chain_",paste(organs[i],cell_types_categories[k],"old"),".Rds")
      # Young Markov chain file name
      young_file = paste0("/tmp/chains","chain_",paste(organs[i],cell_types_categories[k],"young"),".Rds")
      ...
      chain_old = BASiCS_LoadChain(RunName = paste(organs[i],cell_types_categories[k],"old"),StoreDir = "/tmp/chains")
      chain_young = BASiCS_LoadChain(RunName = paste(organs[i],cell_types_categories[k],"young"),StoreDir = "/tmp/chains")
    ...
    # Figure 3 data
    DF_noise = rbind(DF_noise,data.frame("Organs" = organs[i],"Cell_type" = cell_types_categories[k],Old_sign,Young_sign))
    ...
    DF_cor_selc_disp = rbind(DF_cor_selc_disp,data.frame("Organs" = organs[i],"Cell_type" = cell_types_categories[k],
                                                         "selc_disp_old_cor_spearman" = selc_disp_cor_old_spearman,"selc_disp_young_cor_spearman" = selc_disp_cor_young_spearman,
                                                         "selc_disp_cor" = selc_disp_cor,"selc_disp_diff_cor" = selc_disp_log_fc_cor,"selc_disp_fc_cor" = selc_disp_fc_cor,p_val_old,p_val_young))
    ...
    DF_cor_len_disp = rbind(DF_cor_len_disp,data.frame("Organs" = organs[i],"Cell_type" = cell_types_categories[k],len_disp_cor,
                                                       "len_disp_old_cor_spearman" = len_disp_cor_old_spearman,"len_disp_young_cor_spearman" = len_disp_cor_young_spearman,
                                                       len_disp_fc_cor,len_disp_log_fc_cor,log_fc_pval,p_val_old,p_val_young))
    ...
    df_fc_reg_facs = rbind(df_fc_reg_facs,data.frame("Organs" = organs[i],"Cell_type" = cell_types_categories[k],
    ...
    df_selc_reg_facs = rbind(df_selc_reg_facs,data.frame("Organs" = organs[i],"Cell_type" = cell_types_categories[k],
    ...
    # above linear regression data
    df_len_reg_facs = rbind(df_len_reg_facs,data.frame("Organs" = organs[i],"Cell_type" = cell_types_categories[k],
    ...
    selc_OD_reg_data_sm = rbind(selc_OD_reg_data_sm,data.frame("Organs" = organs[i],"Cell_type" = cell_types_categories[k],
    ...
    len_OD_reg_data_sm = rbind(len_OD_reg_data_sm,data.frame("Organs" = organs[i],"Cell_type" = cell_types_categories[k],
