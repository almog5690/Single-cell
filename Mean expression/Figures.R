...
highlight_cell = which(DF_cor$Organs == "Lung" & DF_cor$Cell_type == "type II pneumocyte")
...
p3 = ggplot(DF_cor,aes(selc_mean_young_cor_spearman,selc_mean_old_cor_spearman)) + 
  ...
  geom_text(data = textdf, aes(x = selc_mean_young_spearman * 1.12, y = selc_mean_old_cor_spearman * 0.91, label = "Pneumocyte")) +
...
i = which(organs == "Lung") # Lung index
cell_types_categories = meta.data[[i]]$cell_ontology_class # the names of the different cell-types
k = which(cell_types_categories == "type II pneumocyte") # type II pneumocyte cell index

SC = readRDS(file = paste(organs[i],"rds",sep = ".")) # Lung tissue seurat object
...
## droplet
# Choosing Lung's type 2 pneumocyte cell type to highlight
highlight_cell = which(DF_cor_drop$Organs == "Lung" & DF_cor_drop$Cell_type == "type II pneumocyte")
...
p3_drop = ggplot(DF_cor_drop,aes(selc_mean_young_spearman,selc_mean_old_cor_spearman)) + 
  geom_point(color = "blue") +
  geom_text(data = textdf, aes(x = selc_mean_young_spearman * 1.03, y = selc_mean_old_cor_spearman * 0.89, label = "type II pneumocyte")) +
  geom_abline(slope = 1,intercept = 0,col = "red") +
  geom_point(data = textdf,aes(x = selc_mean_young_spearman , y = selc_mean_old_cor_spearman),color = "orangered",fill = "orange",size = 3) +
...
i = which(drop_organs == "Lung") # Lung index
